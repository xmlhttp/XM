...
void lwip_comm_default_ip_set(__lwip_dev *lwipx){
	...
	if(my_data.Idc==170){//如果开始地址为0000000010101010
		//默认远端IP为:192.168.1.106
		lwipx->remoteip[0]=my_data.Cloud[0];	
		lwipx->remoteip[1]=my_data.Cloud[1];
		lwipx->remoteip[2]=my_data.Cloud[2];
		lwipx->remoteip[3]=my_data.Cloud[3];
		...
		lwipx->ip[0]=my_data.Ip[0];
		lwipx->ip[1]=my_data.Ip[1];
		lwipx->ip[2]=my_data.Ip[2];
		lwipx->ip[3]=my_data.Ip[3];
		//默认子网掩码:255.255.255.0
		lwipx->netmask[0]=my_data.Mask[0];
		lwipx->netmask[1]=my_data.Mask[1];
		lwipx->netmask[2]=my_data.Mask[2];
		lwipx->netmask[3]=my_data.Mask[3];
		//默认网关:192.168.1.1
		lwipx->gateway[0]=my_data.Gway[0];	
		lwipx->gateway[1]=my_data.Gway[1];	
		lwipx->gateway[2]=my_data.Gway[2];	
		lwipx->gateway[3]=my_data.Gway[3];	 
	}else{
		//默认远端IP为:192.168.1.106
