...
class LogController extends Controller {
	...
	public function search(){
		...
		if(session("adminclass")==0){
			...
			$count=M('sys_note')->where("act like '%".I("post.searchtxt",'')."%' or login_name like '%".I("post.searchtxt",'')."%'")->count();
			$T=M('sys_note')->where("act like '%".I("post.searchtxt",'')."%' or login_name like '%".I("post.searchtxt",'')."%'")-> order('id desc')->limit($page*$size,$size)->select();
		}
		$json['pagecount']=ceil($count/$size);
		$json['pagecurrent']=$page;
		$json['data']['rows']=showitem($T);
